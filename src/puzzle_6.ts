...
function isPointOnLine(point: Point, line: Line): boolean {
  return (
    (line.start.y == line.end.y && line.end.y == point.y && line.start.x <= point.x && point.x <= line.end.x) ||
    (line.start.x == line.end.x && line.start.x == point.x && line.start.y <= point.y && point.y <= line.end.y)
  );
}
